		...
		then
			...
			then
				...
				then
					...
					#
					## Old approach with partclone.dd
					# partclone.dd -s "${DEVICE_FILE}" -o - -z 10485760 -N  | 7z a -bd -t7z "${BACKUP_FILE}" -si -mx"${SEVEN_COMP_LEVEL}" -mmt"${THREADS}" 1>/dev/null
					## New approach with partclone.imager
					partclone.imager -c -s "${DEVICE_FILE}" -o - -z 10485760 -N  | 7z a -bd -t7z "${BACKUP_FILE}" -si -mx"${SEVEN_COMP_LEVEL}" -mmt"${THREADS}" 1>/dev/null
					#
				else
		...
		then
			...
			then
                ...
                then
					...
					#
					7z x -bd -so "${BACKUP_FILE}" | partclone.info -s - >/dev/null 2>&1
					TMP="$?"
					if [[ "${TMP}" != "0" ]];
					then
						## Old approach with partclone.dd
						...
						## New approach with partclone.imager
						7z x -bd -so "${BACKUP_FILE}" | partclone.imager -r -s - -o "${DEVICE_FILE}" -z 10485760 -N
					fi
                else
...
function CheckImages
{
	...
	do
		...
		if [[ ! "${TARGET_PARTITIONS[@]}" =~ "${PART}" ]];
		then
			echo "${BACKUP_FILE_NAME} file has been found in selected backup directory, but it will be not checked."
			echo "########################################################################################################################"
			continue
		fi
		#
